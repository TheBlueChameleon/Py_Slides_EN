...
\item \inPy{else} with \inPy{for} and \inPy{while}
	\begin{itemize}
	\item \inPy{for}   -- execute code only when end of loop is reached
	\item \inPy{while} -- execute code only when loop body was never executed
	\end{itemize}
\end{itemize}
%
\column{.5\linewidth}
...
\end{codebox}
%
\begin{codebox}[Syntax: Calling Funcs with opt. Arguments]
\begin{minted}[fontsize=\scriptsize]{python}
funcName (param)
...
\scriptsize
The previous example should only illistrate an idea. Long  \inPy{if-elif-...}-chains can be replaced by \inPy{dict}s:
